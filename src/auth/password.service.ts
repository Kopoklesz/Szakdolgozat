...
export class PasswordService {
   ...
   * Minimum 6 karakter, 1+ nagybetű, 1+ szám, 1+ speciális karakter (pont)
   */
  validatePasswordComplexity(password: string): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
    
    if (password.length < 6) {
      errors.push('A jelszónak legalább 6 karakter hosszúnak kell lennie');
    }
    ...
    if (!/[.]/.test(password)) {
      errors.push('A jelszónak tartalmaznia kell legalább egy pontot (.)');
    }
  ...
  validateEmailDomain(email: string): { isValid: boolean; error?: string } {
    ...
    if (!isValidDomain) {
      return {
        isValid: false,
        error: 'Csak egyetemi email címekkel lehet regisztrálni (@student.uni-pannon.hu vagy @teacher.uni-pannon.hu)'
      };
    }
